"""Step 4: Generate video clips using Veo3.1"""
...
async def generate_single_video(
    ...
    Args:
        start_frame: Path to edited start frame
        end_frame: Path to edited end frame
    ...
    try:
        ...
        start_url = await fal_client.upload_file(start_frame)
        end_url = await fal_client.upload_file(end_frame)
...
async def generate_video_clips(
    ...
    Args:
        edited_frames: Edited frame data from step 3
    ...
    for frame_data in edited_frames:
        clip_index = frame_data["clip_index"]

        logger.info(f"Generating video for clip {clip_index}")

        try:
            output_path = work_dir / f"clip_{clip_index:03d}_generated.mp4"

            generated_path = await generate_single_video(
                frame_data["start_frame_edited"],
                frame_data["end_frame_edited"],
                frame_data["duration"],
                str(output_path),
                prompt,
                config,
